...
class GraphFromRDFId_LatentCase(val source: String, var features : ListBuffer[String]) extends GraphGenerator {

  var SOMap = collection.mutable.Map[String, String]()
  var PMap = collection.mutable.Map[String, String]()
  var feature_objects = ListBuffer[String]()
  ...
  override def get: Graph[String, LkDiEdge] = {
    ...
    modelWithLiteral.listStatements().asScala.foreach(stmt => {
      if (!stmt.getObject.isInstanceOf[Literal]) {
        model.add(stmt)
        val temp = List(stmt.getPredicate.toString())

        if(temp.intersect(features.seq).length==1)
          {
              feature_objects += mapStringtoId(stmt.getObject.toString, SOMap,false)
          }
      }
    })
